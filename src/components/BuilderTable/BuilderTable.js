...
export const BuilderTable = ({ rows, onAddRow, onDeleteRow, onSelectColumn, onToggleRowIndentation }) => {
    let rowElements = <p>No rows.</p>;

    if (rows.length) {
        rowElements = rows.map(({ rowId, colIndex, indent }, rowIndex) => {
            const indentCol = (
                <>
            ...
            return (
                <div className="builderRow" key={rowIndex}>
                    ...
                    <div className="indentCol">{indentCol}</div>
